...
ParallelAsyncTask::ParallelAsyncTask(std::shared_ptr<AsyncTaskManager> async_task_manager, std::function<void()> callback, size_t max_parallel_tasks)
	: AsyncTask(callback)
...
ParallelAsyncTask::ParallelAsyncTask(std::shared_ptr<AsyncTaskManager> async_task_manager, std::vector<std::unique_ptr<AsyncTask>>&& tasks, std::function<void()> callback, size_t max_parallel_tasks)
	: ParallelAsyncTask(async_task_manager, callback, max_parallel_tasks)
{
...
void ParallelAsyncTask::UpdateTasks()
{
	if (m_is_shut_down) return;

	size_t num_tasks_to_start = (m_tasks.size() - m_tasks_started_count) > m_max_parallel_tasks ? m_max_parallel_tasks : (m_tasks.size() - m_tasks_started_count);
