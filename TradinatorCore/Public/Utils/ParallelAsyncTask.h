...
{
public:
	ParallelAsyncTask(std::shared_ptr<AsyncTaskManager> async_task_manager, std::function<void()> callback, size_t max_parallel_tasks = 32);
	ParallelAsyncTask(std::shared_ptr<AsyncTaskManager> async_task_manager, std::vector<std::unique_ptr<AsyncTask>>&& tasks, std::function<void()> callback, size_t max_parallel_tasks = 32);
