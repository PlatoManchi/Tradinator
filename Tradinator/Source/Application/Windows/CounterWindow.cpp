...
void CounterWindow::Show()
{
    ...
    if (ImGui::Begin(m_cached_label_id.c_str(), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse))
    {
        ...
        const std::shared_ptr<const AsyncCandleData>& candle_data = m_counter->GetCandleData();
        ...
        else
        {
            ...
            if (ImPlot::BeginPlot("Candlestick Chart", ImVec2(-1, -1))) {
                ...
                PlotCandlestick(m_counter->Name().c_str(), dates, opens, closes, lows, highs, 218, tooltip, 0.25f, bullCol, bearCol);
                ImPlot::EndPlot();
            }
